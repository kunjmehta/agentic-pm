...
class TestVWAPReversionSkill:
    ...
    def _import(self):
        from src.semi_auto.skills.quant.vwap_reversion import VWAPReversionSkill
...
class TestOpeningRangeBreakoutSkill:
    @pytest.fixture(autouse=True)
    def _import(self):
        from src.semi_auto.skills.quant.opening_range_breakout import OpeningRangeBreakoutSkill
...
class TestRSIDivergenceScalpSkill:
    @pytest.fixture(autouse=True)
    def _import(self):
        from src.semi_auto.skills.quant.rsi_divergence_scalp import RSIDivergenceScalpSkill
...
class TestMomentumBurstSkill:
    @pytest.fixture(autouse=True)
    def _import(self):
        from src.semi_auto.skills.quant.momentum_burst import MomentumBurstSkill
...
class TestGoldenCrossSkill:
    @pytest.fixture(autouse=True)
    def _import(self):
        from src.semi_auto.skills.quant.golden_cross import GoldenCrossSkill
...
class TestBreakout52WeekSkill:
    @pytest.fixture(autouse=True)
    def _import(self):
        from src.semi_auto.skills.quant.breakout_52w import Breakout52WeekSkill
...
class TestMeanReversionDailySkill:
    @pytest.fixture(autouse=True)
    def _import(self):
        from src.semi_auto.skills.quant.mean_reversion_daily import MeanReversionDailySkill
...
class TestEarningsDriftSkill:
    @pytest.fixture(autouse=True)
    def _import(self):
        from src.semi_auto.skills.quant.earnings_drift import EarningsDriftSkill
...
class TestFactoryFunctions:
    ...
    def test_make_vwap_reversion_signals(self):
        from src.semi_auto.skills.quant.vwap_reversion import make_vwap_reversion_signals
        fn = make_vwap_reversion_signals()
        assert fn("AAPL", self._df()) in ("buy", "sell", "hold")

    def test_make_opening_range_breakout_signals(self):
        from src.semi_auto.skills.quant.opening_range_breakout import make_opening_range_breakout_signals
        fn = make_opening_range_breakout_signals(range_bars=10)
        assert fn("AAPL", self._df()) in ("buy", "sell", "hold")

    def test_make_rsi_divergence_signals(self):
        from src.semi_auto.skills.quant.rsi_divergence_scalp import make_rsi_divergence_signals
        fn = make_rsi_divergence_signals()
        assert fn("AAPL", self._df(100)) in ("buy", "sell", "hold")

    def test_make_momentum_burst_signals(self):
        from src.semi_auto.skills.quant.momentum_burst import make_momentum_burst_signals
        fn = make_momentum_burst_signals()
        assert fn("AAPL", self._df()) in ("buy", "sell", "hold")

    def test_make_golden_cross_signals(self):
        from src.semi_auto.skills.quant.golden_cross import make_golden_cross_signals
    ...
    def test_make_breakout_52w_signals(self):
        from src.semi_auto.skills.quant.breakout_52w import make_breakout_52w_signals
        fn = make_breakout_52w_signals(lookback=30)
        assert fn("AAPL", self._df(60)) in ("buy", "sell", "hold")

    def test_make_mean_reversion_daily_signals(self):
        from src.semi_auto.skills.quant.mean_reversion_daily import make_mean_reversion_daily_signals
        fn = make_mean_reversion_daily_signals()
        assert fn("AAPL", self._df(60)) in ("buy", "sell", "hold")

    def test_make_earnings_drift_signals(self):
        from src.semi_auto.skills.quant.earnings_drift import make_earnings_drift_signals
...
class TestBackwardCompatReexport:
    ...
    def test_identity_match(self):
        ...
        from src.semi_auto.skills.quant.skills import VWAPReversionSkill as A
        from src.semi_auto.skills.quant.vwap_reversion import VWAPReversionSkill as B
        assert A is B

        from src.semi_auto.skills.quant.skills import GoldenCrossSkill as C
        from src.semi_auto.skills.quant.golden_cross import GoldenCrossSkill as D
        assert C is D
