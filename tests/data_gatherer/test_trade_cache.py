...
from src.data_gatherer.trade_cache import TradeCache, reset_cache
...
class TestTradeCache:
    ...
    def test_size_based_flush_trigger(self, sample_trade):
        ...
        # Add 90 trades (90% of 100)
        for i in range(89):
            cache.add_trade(sample_trade.copy())

        # 90th trade should trigger flush
        should_flush = cache.add_trade(sample_trade)
    ...
    def test_max_size_enforcement(self, sample_trade):
        """Test that cache respects maxlen (deque automatically drops oldest)."""
        cache = TradeCache(batch_interval_minutes=60, max_size=10)

        # Add 15 trades (exceeds max_size)
        for i in range(15):
            trade = sample_trade.copy()
            trade['trade_id'] = i
            cache.add_trade(trade)

        # Cache should only have 10 trades (oldest dropped)
        assert cache.get_current_size() == 10
