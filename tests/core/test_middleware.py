...
class TestCreateMiddlewareStack:
    ...
    def test_quant_stack(self):
        ...
        assert len(stack) == 3
        assert isinstance(stack[0], MarketHoursGuardMiddleware)
        assert isinstance(stack[1], TracingMiddleware)
        assert isinstance(stack[2], PrettifyMiddleware)
    ...
    def test_portfolio_stack(self):
        ...
        assert len(stack) == 4
        assert isinstance(stack[0], MarketHoursGuardMiddleware)
        assert isinstance(stack[1], PortfolioGuardMiddleware)
    ...
    def test_backtest_mode_propagates(self):
        ...
        assert stack[0].backtest_mode is True
