...
if __name__ == "__main__":
    print("=" * 60)
    print("BacktestDAO Functional Test")
    print("=" * 60)

    # Initialize DAO
    dao = BacktestDAO()
    print("\n[OK] DAO initialized, schema loaded")

    # Test 1: Create a backtest run
    ...
    trade_id = dao.save_trade(
        ...
        quantity=100,
        side="long",
        entry_signal={"z_score": -2.1, "sma": 148.50}
    )
    print(f"   Created trade: {trade_id}")

    # Test 3: Close the trade
    print("\n3. Closing trade...")
    dao.close_trade(
        trade_id=trade_id,
        exit_date=date(2024, 1, 16),
        exit_time=datetime(2024, 1, 16, 14, 30),
        exit_price=152.75,
        exit_reason="take_profit"
    )
    print("   [OK] Trade closed with P&L calculated")

    # Test 4: Save daily performance
    print("\n4. Saving daily performance...")
    dao.save_daily_performance(
        run_id=run_id,
        date=date(2024, 1, 15),
        equity=100250.0,
        cash=85000.0,
        positions_value=15025.0,
        daily_pnl=250.0,
    ...
    print("   [OK] Performance saved")

    # Test 5: Update run performance
    print("\n5. Updating run performance metrics...")
    dao.update_run_performance(
        run_id=run_id,
        metrics={
    ...
    if not trades.empty:
        print(f"   First trade: {trades.iloc[0]['symbol']} {trades.iloc[0]['side']} @ ${trades.iloc[0]['entry_price']}")

    # Test 9: Get performance history
    print("\n9. Retrieving performance history...")
    performance = dao.get_performance_history(run_id)
    print(f"   Found {len(performance)} daily snapshots")

    # Test 10: Get recent runs
    print("\n10. Retrieving recent runs...")
    recent_runs = dao.get_recent_runs(limit=5)
