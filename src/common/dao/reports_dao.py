...
if __name__ == "__main__":
    print("=" * 60)
    print("ReportsDAO smoke test")
    print("=" * 60)

    dao = ReportsDAO()
    print("[OK] Schema ensured")

    # Test signal tracking insert
    tid = dao.save_signal_tracking(
        signal_id=999,
        symbol="AAPL",
    ...
    print(f"[OK] save_signal_tracking → id={tid}")

    rows = dao.get_signal_performance(symbol="AAPL")
    assert any(r["signal_id"] == 999 for r in rows), "Row not found"
    print(f"[OK] get_signal_performance → {len(rows)} row(s)")

    dao.close_tracking_row(signal_id=999, exit_price=180.0, realized_pnl=50.0)
    print("[OK] close_tracking_row")

    # Test hypothetical upsert
    dao.upsert_hypothetical(
        snapshot_date=date.today(),
        strategy_name="mean-reversion",
        symbol="AAPL",
        hypothetical_value=105000.0,
        actual_value=103000.0,
        signals_taken=2,
        signals_missed=1,
        opportunity_pnl=2000.0,
    )
    hypo = dao.get_hypothetical_vs_actual(str(date.today()))
    assert hypo, "Hypothetical row not found"
    print(f"[OK] hypothetical upsert + read → {len(hypo)} row(s)")

    # Cleanup
    dao.execute("DELETE FROM signal_performance_tracking WHERE signal_id = 999")
    dao.execute("DELETE FROM hypothetical_portfolios WHERE symbol = 'AAPL' AND strategy_name = 'mean-reversion'")
    dao.close()
    print("=" * 60)
