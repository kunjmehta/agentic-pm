...
class PortfolioDAO(BaseDAO):
    ...
    def save_snapshot(
        ...
        Args:
            ...
            positions: Optional list of position dicts (for backtesting/swap analysis)
    ...
    def save_thread_id(self, thread_id: str) -> None:
        """Register a conversation thread ID.

        Inserts the thread_id if new; updates updated_at if already exists.

        Args:
            thread_id: Client-generated UUID for the conversation thread
        """
        conn = self.connect()
        conn.execute(
            """
        ...
        logger.debug(f"Saved thread_id: {thread_id}")
