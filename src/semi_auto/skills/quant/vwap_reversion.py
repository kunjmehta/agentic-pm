    ...
    def analyze_bars(
        ...
        if abs(deviation) >= dev_pct and vol_ok:
            ...
            reason = (
                ...
                f"with {cur_vol / avg_vol:.1f}× volume spike"
            )
            return {
                "action": action,
...
def make_vwap_reversion_signals(dev_pct: float = 0.005, vol_mult: float = 2.0) -> Callable:
    ...
    def signal_fn(symbol: str, bars_df: pd.DataFrame) -> str:
        if bars_df is None or bars_df.empty:
            return "hold"
        return _skill.analyze_bars(bars_df, dev_pct=dev_pct, vol_mult=vol_mult).get("action", "hold")
